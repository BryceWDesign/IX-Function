...
def test_validate_learning_inputs_blocks_lineage_mismatch() -> None:
    ...
    errors = validate_learning_inputs(make_function(), report)

    assert "causal_function function_id must match report function_id" in errors
...
def test_build_learning_update_promotes_supported_transfer() -> None:
    ...
    assert update.function_id == "causal-bottleneck-v1"
